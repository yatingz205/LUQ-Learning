...
cond_exp = function(
  ...
  # x and y components
  comps_x1_mat = cbind(dbinom(x1[, 1], 10, prob = 0.5), dbinom(x1[, 2], 10, 0.5), dbinom(x1[, 3], 10, 0.5))
...
pi2_fnc = function(
  ...
  if (length(level) == 1) {
    result = rep(level[1], nrow(x1_new))
    return(result)
  }
...
evaluateValue = function(
  pi1_map, pi2_map, n=100, seed=42, lab='op', run='op_seed=42_n=600',
  alpha01, alpha1, alpha02, alpha2, 
  beta01, beta1, beta02, beta2, 
