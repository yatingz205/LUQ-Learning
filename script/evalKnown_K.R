...
piK = function(
  ...
  aK = A[,,K]; xK = X[,,K];
  HK = data.frame(X=X[,,1:K], A=A[,,1:K-1], V=V)
...
pik = function(
  ...
  for (j in 1:length(ak_combos)) {
    ...
    if(k>1) {
      covars_j = data.frame(X=X[,,1:k], A=A[,,1:k-1], V=V, ak = ak_broad)
      colnames(covars_j) <- gsub("\\.+", "_", colnames(covars_j))
    } else {
      covars_j = data.frame(X=X[,,1:k], V=V, ak = ak_broad)
      colnames(covars_j) <- gsub("\\.+", "_", colnames(covars_j))
    }
